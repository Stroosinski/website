...
function loadThree() {
  if (!threeModPromise) threeModPromise = import("https://esm.sh/three@0.166.1").then((mod) => { THREE = mod; return mod; });
...
function createVortex(canvas, container, cfgRef) {
  ...
  function build() {
    ...
    const strandMat = track(
      withRepel(new THREE.LineBasicMaterial({ vertexColors: true, transparent: true, opacity: 0.5, blending: THREE.AdditiveBlending, depthWrite: false }))
    );
    ...
    if (dotCount > 0) {
      ...
      const dotMat = track(
        withRepel(new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.9, blending: THREE.AdditiveBlending, depthWrite: false }))
      );
    ...
    for (let i = 0; i < cometTotal; i++) {
      ...
      const lineMat = track(
        withRepel(new THREE.LineBasicMaterial({ vertexColors: true, transparent: true, opacity: 0.9, blending: THREE.AdditiveBlending, depthWrite: false }))
      );
  ...
  let frame = 0, born = 0, elapsed = 0, flow = 0, strandClock = 0, heldFrame = false, tick = 0;
  ...
  function step(now) {
    ...
    if (dotMesh && dotCount > 0) {
      ...
      for (let i = 0; i < dotCount; i++) {
        ...
        const beat = 1 - cfg.dotFlicker + cfg.dotFlicker * (0.08 + 0.92 * Math.max(0, Math.sin(t * dot.flickerRate + dot.pulse)) ** 2.5);
...
function Vortex(props) {
  ...
  return (
    <div
      ref={containerRef}
      style={{ ...style, position: "relative", width: "100%", height: "100%", background, overflow: "hidden" }}
    >
