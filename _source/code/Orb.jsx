const DEFAULTS = {
  ...
  archetype: "nebula",
  background: "#0A0A0A",
  palette: { anchor: "#000000", colorA: "#FDE300", colorB: "#EAF100", colorC: "#D6B460" },
  speed: 14,
  spin: 98,
  lens: true,
  lensAmount: 59,
...
const UNIFORM_NAMES = ["uRes", "uBg", "uAnchor", "uC0", "uC1", "uC2", "uTime", "uPhase", "uAudio", "uSpin", "uArch", "uLens"];
...
function Orb(props) {
  ...
  React.useEffect(() => {
    ...
    const init = () => {
      ...
      prog = p;
      u = {};
      for (const name of UNIFORM_NAMES) u[name] = gl.getUniformLocation(p, name);
    ...
    const body = { spin: 0, spinVel: 0, spinDir: 1, oscSign: 1, flipQueued: false, audioSmooth: 0, audioFast: 0, prevA: 0, lastT: null };

    const advance = (tNow, cfg) => {
  ...
  return (
    ...
    >
      ...
      >
        <canvas ref={canvasRef} style={{ display: "block", width: "100%", height: "100%", borderRadius: "50%" }} />
      </div>
    </div>
  );
}
